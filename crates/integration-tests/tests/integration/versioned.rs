...
/// Rewrite the responses for websocket endpoints in an OpenAPI document from the
/// new dropshot 0.17 format (101/4XX/5XX) back to the old 0.16 format (default
/// with */* schema). This simulates blessed documents generated by pre-0.17
/// dropshot.
fn rewrite_ws_responses_to_old_format(spec_json: &str) -> String {
    ...
    for (_path, item) in paths.iter_mut() {
        ...
        for (_method, operation) in item.iter_mut() {
            let op = match operation.as_object_mut() {
                Some(op) => op,
                None => continue,
            };
            // Only rewrite operations with x-dropshot-websocket.
            if !op.contains_key("x-dropshot-websocket") {
                continue;
            }
            // Replace the responses with old-format default response.
            op.insert(
                "responses".to_string(),
                json!({
                    "default": {
...
/// Compute the 6-character content hash used in versioned spec filenames.
/// Mirrors `hash_contents` in dropshot-api-manager.
fn content_hash(contents: &[u8]) -> String {
    let mut hasher = Sha256::new();
    hasher.update(contents);
    let hash = hasher.finalize();
    hex::encode(&hash.as_slice()[0..3])
}
...
fn rewrite_versioned_doc_ws_format(
    ...
    let content = env.read_file(&doc_path)?;
    let rewritten = rewrite_ws_responses_to_old_format(&content);

    // Compute new hash and build new filename.
    let new_hash = content_hash(rewritten.as_bytes());
    let new_filename = format!("{}-{}-{}.json", api_ident, version, new_hash);
    let new_path =
        Utf8PathBuf::from(format!("documents/{}/{}", api_ident, new_filename));

    // Delete old file, write new one.
    std::fs::remove_file(env.workspace_root().join(&doc_path))?;
    env.create_file(&new_path, &rewritten)?;

    // If latest symlink points to old file, update it.
    let latest_link =
        format!("documents/{}/{}-latest.json", api_ident, api_ident);
    if env.file_exists(&latest_link) {
        let target = env.read_link(&latest_link)?;
        let old_filename = doc_path.file_name().expect("should have filename");
        if target.as_str() == old_filename {
...
fn test_blessed_ws_old_format_fails_without_normalizer() -> Result<()> {
    let env = TestEnvironment::new_git()?;
    let apis = versioned_ws_apis()?;

    // Generate the documents (new 0.17 format).
    env.generate_documents(&apis)?;

    // Read the generated v1 spec, rewrite it to old format, and compare
    // directly with drift (no normalizer) to prove incompatibility.
    let doc_path = env
        .find_versioned_document_path("versioned-ws", "1.0.0")?
        .expect("v1 doc should exist");
    let generated_content = env.read_file(&doc_path)?;
    let old_format_content =
        rewrite_ws_responses_to_old_format(&generated_content);

    let generated: serde_json::Value =
        serde_json::from_str(&generated_content)?;
    let old_format: serde_json::Value =
        serde_json::from_str(&old_format_content)?;

    // Raw drift comparison (no normalizer) should find issues.
    let changes = drift::compare(&old_format, &generated)?;
...
fn test_blessed_ws_old_format_normalized_older_versions_pass() -> Result<()> {
    ...
    // Rewrite both version documents to old websocket format, renaming
    // files to match the new content hash.
    rewrite_versioned_doc_ws_format(&env, "versioned-ws", "1.0.0")?;
    rewrite_versioned_doc_ws_format(&env, "versioned-ws", "2.0.0")?;

    // Commit the old-format documents as blessed.
    env.commit_documents()?;
...
fn test_blessed_ws_old_format_passes_with_trivial_changes_allowed() -> Result<()>
{
    ...
    // Use the standard config for initial generation.
    let apis = versioned_ws_apis()?;
    env.generate_documents(&apis)?;

    // Rewrite to old format and commit.
    rewrite_versioned_doc_ws_format(&env, "versioned-ws", "1.0.0")?;
    rewrite_versioned_doc_ws_format(&env, "versioned-ws", "2.0.0")?;
    env.commit_documents()?;

    // Build an API config with allow_trivial_changes_for_latest.
